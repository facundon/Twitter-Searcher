import tweepy
import json
from EjercicioMauro.settings import TWITTER_API_KEYS
...
def make_search(term):
    tweets = dict()
    for number, tweet in enumerate(tweepy.Cursor(api.search, q=term, tweet_mode="extended").items(10), start=1):
